        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div class="lg:col-span-7">
                <div class="bg-white shadow rounded-lg overflow-hidden">
                    ...
                    <div class="p-6">
                        <form action="{{ route('edit_post_submit',['id'=>$post->id]) }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label for="content" class="block text-xl font-medium text-gray-700">{{__('language.create_post_title')}}</label>
                                <input placeholder="{{__('language.create_post_title_placeholder')}}" type="text" id="title" name="title" value="{{$post->title}}"
                            ...
                            <div class="mb-4">
                                ...
                                <textarea type="text" id="description" name="description"
                                    ...
                                    placeholder="{{__('language.create_post_description_placeholder')}}" required>{{$post->description}}</textarea>
                            </div>
                            <div class="mb-4">
                                <label for="content" class="block text-xl font-medium text-gray-700">{{__('language.create_post_content')}}</label>
                                <textarea id="editor" name="content" class="w-full h-full">{{$post->content}}</textarea>
                            </div>
                            <div class="mb-4">
                                <button type="submit"
                                    class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">{{__('language.setting_edit_post_save_changes')}}</button>
                            </div>
                        </form>
                    </div>
                </div>
